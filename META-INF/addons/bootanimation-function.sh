...
ba() {
    ...
    if $find; then
       ...
       #Check's how many times the boot animation is found then deletes all but the first boot animation found
       if [ $(find /system -type f -name bootanimation.zip | grep -c "bootanimation.zip") -ge 2 ]; then
